...
function exit(exitCode) {
	...
	//Codigo para actualizar controlies.	
		
	UpdateControlies(exitCode);
...
function lastboottime() {
	...
	for (; !enumItems.atEnd(); enumItems.moveNext()) {
		...
		var oI = enumItems.item();
        t = oI.LastBootUpTime;
  	    lastboot=t.substr(0,4)+ "-" + t.substr(4,2) + "-" + t.substr(6,2) + " " + t.substr(8,2) + ":" + t.substr(10,2) + ":" + t.substr(12,2);
	}
	return lastboot;
}
...
function ahora() {
    ...
    return t.getFullYear()+"-"+pad(t.getMonth()+1,2)+"-"+pad(t.getDate(),2)+" "+pad(t.getHours(),2)+":"+pad(t.getMinutes(),2)+":"+pad(t.getSeconds(),2);
}


function wgetupload(url, texto) {
	var objXMLHTTP = new ActiveXObject("MSXML2.XMLHTTP");
	objXMLHTTP.open("POST", url, false);
        objXMLHTTP.setRequestHeader("User-Agent", "Mozilla/5.0");
	objXMLHTTP.send(texto);
	return objXMLHTTP.Status;
}


function procesarxmlwpkg(fichero) {

  var xml = new ActiveXObject("Microsoft.XMLDOM");
  var texto="";
  //xml.Async = "False";
  fichero.Position=0;
  mensaje=fichero.ReadText(-1);
  xml.loadXML(mensaje);
  
  x=xml.documentElement.childNodes;
  for (i=0;i<x.length;i++)
  { 
      if (x[i].nodeName=="package"){
			 texto=texto+"-"+x[i].getAttribute("id")+"\r";
            } 
  }  
  return texto;
}  
...
function getInstalledSoftware(host) {
   
	var objWMIService = GetObject("winmgmts:{impersonationLevel=impersonate}!\\\\.\\root\\cimv2");
	var colSoftware = objWMIService.ExecQuery ("Select * from Win32_Product");
	var colSettings = objWMIService.ExecQuery ("Select * from Win32_OperatingSystem");
	var texto="";
	
	var enumItems = new Enumerator(colSettings);
	for (; !enumItems.atEnd(); enumItems.moveNext()) {
		var oI = enumItems.item();
        pos=oI.Name.indexOf("|");
		name=oI.Name.substr(0,pos);
		servicepack=oI.ServicePackMajorVersion + "." + oI.ServicePackMinorVersion;
		texto=texto+name+" SP"+servicepack+"\r";
     }		
	 
	var enumItems = new Enumerator(colSoftware);
	for (; !enumItems.atEnd(); enumItems.moveNext()) {
		var oI = enumItems.item();
		fecha=oI.InstallDate;
		fecha= fecha.substr(6,2) + "/" +  fecha.substr(4,2) + "/" + fecha.substr(0,4); 
        texto=texto+"-"+oI.Caption+"-"+oI.Version+". Fecha instalacion: "+ fecha +"\r";
     }		
     return texto;  	    
}
...
function crearficherolog(host,exitCode,ahora) {
   var stream=new ActiveXObject("ADODB.Stream");
   var stream2=new ActiveXObject("ADODB.Stream");
   
   //Stream Destino
   stream.Open;
   stream.Type = 2; //adTypeTex
   stream.Charset = "ascii";
   
   
   stream.WriteText (host+":" +ahora+". Finalizado wpkg. Codigo de salida: "+exitCode+"\r\r");
   stream.WriteText ("IP "+getIP()+"\r\r");
   stream.WriteText ("Paquetes WPKG aplicados:\r\r");
   
   //Stream2. Cargamos en el c:\windows\system32\wpkg.xml
   try {

	   stream2.Open;
	   stream2.Type = 2; //adTypeTex
	   stream2.Charset = "ascii";
	   stream2.LoadFromFile("c:\\windows\\system32\\wpkg.xml");
		  //stream2.CopyTo(stream,-1);
	   texto=procesarxmlwpkg(stream2);     
	   stream.WriteText(texto);      
	   stream2.close();
   }    
   catch (e) {
     stream.WriteText("Error accediendo a wpkg.kml: "+e.message +":"+e.number+"\r");
   }
   
   stream.WriteText ("\rLog de la ejecucion:\r");   
   stream.WriteText ("=================================================================================\r");   
   
   //Stream2. Cargamos en el c:\windows\temp\wpkg-host.log
   ...
   try {
        ...
        stream2.Type = 2; //adTypeTex
        stream2.Charset = "ascii";
        fichero="c:\\windows\\temp\\wpkg-"+host+".log";
        stream2.LoadFromFile(fichero);
        stream2.CopyTo(stream,-1);   
        stream2.close();

   } catch (e) {
	      stream.WriteText("Error accediendo a "+fichero+": "+e.message +":"+e.number+"\r");
   }
...
/*
   ...
   stream.WriteText ("Software Instalado:\r");   
   
  
   //texto=getInstalledSoftware(host);
   texto="- Desactivado por el momento";
      stream.WriteText(texto);   
   */
...
function UpdateControlies(exitCode) {
	var objNet = new ActiveXObject("WScript.NetWork");

	url="http://ldap/controlies/init/registro/actualizahost";
	maquina=objNet.ComputerName;
	ultimoarranque=lastboottime();
	ultimowpkg=ahora();
	if(exitCode != 0) { wpkg_status="ERROR"; }
	else { wpkg_status="OK"; }
	
	urllog="http://ldap/controlies/init/registro/actualizalogwpkg/"+maquina;	
	stream=crearficherolog(maquina,exitCode,ultimowpkg);				
	stream.Position = 0;	
    logtext=stream.ReadText(-1);
    /*Si dentro del log aparece la cadena "ERROR    :" se considera que hay un error, aun cuando
     * el exitCode sea == 0
     */
    if (logtext.indexOf("ERROR   :") !== -1) wpkg_status="ERROR";
		
	urlenvio=url + "?host=" + maquina + "&tipohost=WINDOWS&ultimoarranque=" + ultimoarranque + "&ultimopkgsync=" + ultimowpkg + "&estadopaquetes=" + wpkg_status;    
    resultado=wget(urlenvio);
        
    resultado=wgetupload(urllog,logtext);
	stream.Close;
